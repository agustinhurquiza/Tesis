...
def initial_regions(image, scale):
    init_segments = segmentation.felzenszwalb(image, scale=scale,
...
def color_hist(reg_mask, bins=25, lower_range=0.0, upper_range=255.0):
    ...
    for channel in np.arange(reg_mask.shape[1]):
        hist.append(np.histogram(reg_mask[:, channel], bins,
...
def texture_hist(text_reg_mask, bins=80, lower_range=0.0, upper_range=255.0):
    ...
    for channel in np.arange(text_reg_mask.shape[1]):
        hist.append(np.histogram(text_reg_mask[:, channel], bins,
                                 (lower_range, upper_range))[0])
    hist = np.concatenate(hist, axis=0)
    hist_norm = normalize(hist.reshape(1, -1), norm='l1')
...
def extract_regions(img_and_seg):
    ...
    for r in np.unique(segments):
        ...
        R.append({"x_min": x_min, "x_max": x_max, "y_min": y_min, "y_max": y_max,
                  "width": width, "height": height, "size": size, "label": r})
...
def extract_neighbors(img_and_seg, regions):
    ...
    segments = img_and_seg[:, :, 3]
    for r in regions:
        x_min = r["x_min"] - 1 if r["x_min"] != 0 else r["x_min"]
        x_max = r["x_max"] + 2 if r["x_max"] != w else r["x_max"]
        y_min = r["y_min"] - 1 if r["y_min"] != 0 else r["y_min"]
        y_max = r["y_max"] + 2 if r["y_max"] != h else r["y_max"]
...
def merge_regions(img_and_seg, regions, R, N):
    ...
    img_and_seg[:, :, 3][img_and_seg[:, :, 3] == regions[1]] = regions[0]
...
def selective_search(image, colour_space="hsv", scale=20, measure=(1, 1, 1, 1),
                     sim_threshold=0.65):
    """
    Parameters
    ----------
    ...
    :param scale: float
        Free parameter. Higher means larger clusters in the initial segmentation
    ...
    :segment_mask: (width, height) ndarray
        Ndarray with the same width and height that the input image with
        labeled regions.
    :regions: list
